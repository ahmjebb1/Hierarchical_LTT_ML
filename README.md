...
# data
Raw LTT data is processed using the updated [Lotti App](https://github.com/bubblecontrol/lotti_app). 
...
# dataset
A collection of [pickle files](https://docs.python.org/3/library/pickle.html) (.pkl) comprises the preprocessed audio data, repackaged after processing in the normal way with Raven Lite and Lotti. Each file, e.g. `B_GL.pkl`, is a pickle of a list. The list contains numpy arrays, typically of shape (128,300), representing the spectrogram of a call (128 frequency bins and 300 samples). Each list is all the calls for a single bird.
